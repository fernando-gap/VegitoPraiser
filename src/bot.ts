...
import { DatabaseConnection, DatabaseConnectionDevelopment, DatabaseConnectionProduction } from "./database/database-connection.js";
import { DatabaseConnectionVegitoError, DatabaseSetupVegitoError, IncorrectEnvironmentVegitoError } from "./errors.js";
...
export default class Bot extends Client {
    ...
    async init() {
        ...
        if (this.isProd()) {
            ...
            throw new IncorrectEnvironmentVegitoError("Incorrect Node Env", "Expected production or development");
        }
    ...
    private async setupDatabase(connection: DatabaseConnection) {
        try {
            ...
            if (sequelize === undefined) {
                throw new DatabaseConnectionVegitoError("Connection Error", "Cache value is undefined");
            }
            ...
            throw new DatabaseSetupVegitoError("Database Setup Error", "sequelize connection failed or database is down", { cause: e });
        }
    }
    ...
    async connectDev() {
        if (this.dbdev === undefined) {
            await this.setupDatabase(new DatabaseConnectionDevelopment("../../db_dev.env"));
            this.db = this.dbdev!;
        } else {
            this.db = this.dbdev;
        }
