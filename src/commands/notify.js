    ...
    if (notifyType === "hourly") {
        if (isEnabled) {
            ...
            if (user.has_hourly_reminder) {
                ...
                await interaction.bot.scheduler.create("hourly_reminder_praise", {
            ...
            if (!user.has_hourly_reminder) {
                ...
                await interaction.bot.scheduler.delete("hourly_reminder_praise", { user_id: interaction.user.id });
        ...
        if (isEnabled) {
            ...
            if (user.has_daily_reminder) {
                ...
                await interaction.bot.scheduler.reschedule("daily_reminder_praise", {
                    user_id: interaction.user.id,
                    channel_id: interaction.channel.id
                });
                ...
                await interaction.bot.scheduler.create("daily_reminder_praise", {
                    user_id: interaction.user.id,
                    channel_id: interaction.channel.id
                });
            }
            ...
            if (!user.has_daily_reminder) {
                ...
                embed.setDescription("Vegito is still with you! You will no longer receive reminders.\n");
                await interaction.bot.scheduler.delete("daily_reminder_praise", { user_id: interaction.user.id });
                await userDAO.update(interaction.user.id, "has_daily_reminder", false);
            }
