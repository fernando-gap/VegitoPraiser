...
export default class Praise extends VegitoEvent<VegitoCommand> {
    override async handleChatInputCommand(interaction: ChatInputCommandInteraction<CacheType>) {
        if (interaction.channel === null) {
            throw new NullChannelVegitoError("hourly reminder", "the channel cannot be null, (can't run in dms)");
        }

        const contextCooldown: ContextCooldown | boolean = await this.cooldownValidation({
        ...
        try {
            ...
            throw new TransactionVegitoError("praise command", "praise count or praise coins failed to increment", { cause: e });
        }

        if (this.user === undefined) {
            throw new CommandExecutionVegitoError("praise command", "the command needs a database user");
        }
