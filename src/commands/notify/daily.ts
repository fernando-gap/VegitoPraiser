...
export default class Daily extends VegitoEvent<VegitoSubCommand> {
    override async handleChatInputCommand(interaction: ChatInputCommandInteraction<CacheType>): Promise<void> {
        if (this.user === undefined) {
            throw new CommandExecutionVegitoError("daily command", "the command needs a database user");
        }
        ...
        if (isEnabled) {
            ...
            if (this.user.reminderId === ReminderType.DAILY) {
                ...
                if (interaction.channel === null) {
                    throw new NullChannelVegitoError("Channel is null", "cannot have null channel in daily reminders");
                }
                ...
                try {
                    ...
                    throw new TransactionVegitoError("transaction error", "failed to enable daily update reminder");
                }
            ...
            if (this.user.reminderId === ReminderType.DISABLED) {
                ...
                await this.bot.scheduler.delete("notifyDailyPraise", { userId: interaction.user.id });
            }
        }
    }
}
