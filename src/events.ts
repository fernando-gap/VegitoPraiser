...
export default abstract class VegitoEvent<Command extends CommandType> {
  ...
  abstract handleChatInputCommand(
  ...
  setUp(subcommands?: VegitoEvent<VegitoSubCommand>[]): void {
    if (subcommands !== undefined && subcommands.length > 0) {
      if (this.isSubCommand(this.command)) {
        throw new NestedSubCommandVegitoError(
          "nested sub-command",
          "A sub-command cannot have another sub-command",
        );
      }
    ...
    this.bot.on(Events.InteractionCreate, async (interaction) => {
      if ("commandName" in interaction) {
        if (interaction.commandName !== this.command.name) {
          Debug.log(
            `The command ${interaction.commandName} can't be handled by ${this.command.name}`,
          );
          return;
        }
      ...
      if ("commandName" in interaction) {
        ...
        if (interaction.isChatInputCommand()) {
          ...
          if (subcommandName !== this.command.name) {
            Debug.log(
              `The command ${interaction.commandName} can't be handled by sub-command /${this.command.parent.name} ${this.command.name}`,
            );
            return;
          }
  ...
  private async userLoad(interaction: Interaction<CacheType>) {
    try {
      ...
      throw new TransactionVegitoError(
        "create or find user",
        "Unable to create or retrieve new/old user",
        { cause: e },
      );
    }
    ...
    if (this.user === undefined)
      throw new TransactionVegitoError(
        "bug",
        "Transaction completed but user not assigned.",
      );
  }
      ...
      if (this.bot.isDev()) {
        if (!interaction.isAutocomplete()) {
          await interaction.reply(
            oneLine`Worker ${bold("Nandoka")} is currently doing maintenance :3`,
          );
          return;
        }
      }
  ...
  protected async error(interaction: Interaction<CacheType>, e: unknown) {
    const error = new CommandExecutionVegitoError(
      "command execute",
      "the command have thrown an error",
      { cause: e },
    );
    error.report();
  ...
  protected async cooldownValidation(
    dataCooldown: JobDataCooldown,
  ): Promise<ContextCooldown | boolean> {
