...
export default class Scheduler {
    ...
    async reschedule(name: string, newData: any) {
        const job = this.jobs.get(name);
        if (job === undefined)
            throw new JobNotSetupVegitoError("job is undefined", "job was not set up with `add`");
    ...
    async delete(name: string, data: JobData) {
        const jobInstance = await this.drive.jobs({
            name: name, "data.userId": data.userId
        });
        if (jobInstance.length > 0)
            await jobInstance[0].remove();
        else
            throw new RuntimeJobVegitoError("job delete", "cannot delete a job that doesn't exist")
    }
