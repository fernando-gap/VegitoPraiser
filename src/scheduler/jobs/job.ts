...
import {
  JobNotSetupVegitoError,
  NotSchedulableJobVegitoError,
  RuntimeJobVegitoError,
} from "../../errors.js";
...
export default abstract class JobVegito {
    ...
    this.scheduler.define(this.name, async (job) => {
      ...
      if (this.user === null) {
        throw new RuntimeJobVegitoError(
          "jobs needs valid users",
          "the user id for job does not exist or is invalid",
        );
      }

      /* it removes itself because user can only have one job */
      if (
        this.user.reminderId === ReminderType.DISABLED ||
        this.user.reminderId !== ReminderType.HOURLY
      ) {
  ...
  async reschedule(job: Job<any>, data: JobData): Promise<void> {
    throw new NotSchedulableJobVegitoError(
      "scheduler",
      "the job don't override reschedule",
    );
  }
}
